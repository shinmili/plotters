...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let root_area = root_area.titled(&mut backend, "Image Title", ("sans-serif", 60))?;
    ...
    let mut cc = ChartBuilder::on(&upper)
        ...
        .build_cartesian_2d(&mut backend, -3.4f32..3.4, -1.2f32..1.2f32)?;
    ...
    cc.draw_series(
        &mut backend,
        LineSeries::new(x_axis.values().map(|x| (x, x.sin())), &RED),
    )?
    .label("Sine")
    ...
    cc.draw_series(
        &mut backend,
        LineSeries::new(x_axis.values().map(|x| (x, x.cos())), &BLUE),
    )?
    .label("Cosine")
    ...
    for (drawing_area, idx) in drawing_areas.iter().zip(1..) {
        let mut cc = ChartBuilder::on(&drawing_area)
            ...
            .build_cartesian_2d(&mut backend, -1f32..1f32, -1f32..1f32)?;
        cc.configure_mesh()
            .x_labels(5)
            .y_labels(3)
            .max_light_lines(4)
    ...
    root_area.present(&mut backend).expect("Unable to write result to file, please make sure 'plotters-doc-data' dir exists under current dir");
