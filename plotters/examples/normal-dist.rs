...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut x_hist_ctx = ChartBuilder::on(&areas[0])
        ...
        .build_cartesian_2d(
            &mut backend,
            (0.0..1.0).step(0.01).use_round().into_segmented(),
            0..250,
        )?;
    let mut y_hist_ctx = ChartBuilder::on(&areas[3])
        .x_label_area_size(40)
        .build_cartesian_2d(&mut backend, 0..250, (0.0..1.0).step(0.01).use_round())?;
    let mut scatter_ctx = ChartBuilder::on(&areas[2])
        .x_label_area_size(40)
        .y_label_area_size(40)
        .build_cartesian_2d(&mut backend, 0f64..1f64, 0f64..1f64)?;
    scatter_ctx
        .configure_mesh()
        .disable_x_mesh()
        .disable_y_mesh()
    ...
    x_hist_ctx.draw_series(&mut backend, x_hist)?;
    y_hist_ctx.draw_series(&mut backend, y_hist)?;
