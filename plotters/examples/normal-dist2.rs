...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut chart = ChartBuilder::on(&root)
        ...
        .build_cartesian_2d(&mut backend, -4f64..4f64, 0f64..0.1)?
        .set_secondary_coord(
    ...
    chart
        .draw_secondary_series(&mut backend, actual)?
        .label("Observed")
    ...
    chart
        .draw_series(&mut backend, pdf)?
        .label("PDF")
        .legend(|(x, y)| PathElement::new(vec![(x, y), (x + 20, y)], RED.filled()));

    chart.configure_series_labels().draw(&mut backend)?;
