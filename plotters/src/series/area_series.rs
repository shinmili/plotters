...
let mut backend = SVGBackend::new("area_series.svg", (300, 200));
...
chart_context.draw_series(&mut backend, AreaSeries::new(x_values.map(|x| (x, 0.3 * x)), 0., BLACK.mix(0.2))).unwrap();
chart_context.draw_series(&mut backend, AreaSeries::new(x_values.map(|x| (x, 2.5 - 0.05 * x * x)), 0., RED.mix(0.2))).unwrap();
chart_context.draw_series(&mut backend, AreaSeries::new(x_values.map(|x| (x, 2. - 0.1 * x * x)), 0., BLUE.mix(0.2)).border_style(BLUE)).unwrap();
```
