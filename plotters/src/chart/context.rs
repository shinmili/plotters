...
impl<'e, CT: ReverseCoordTranslate> ChartContext<'e, CT> {
...
impl<'e, CT: CoordTranslate> ChartContext<'e, CT> {
    ...
    let mut backend = SVGBackend::new("configure_series_labels.svg", (300, 200));
    ...
    chart_context.draw_series(&mut backend, LineSeries::new(data, BLACK)).unwrap().label("Series 1")
    ...
    chart_context.configure_series_labels().position(SeriesLabelPosition::UpperRight).margin(20)
        .legend_area_size(5).border_style(BLUE).background_style(BLUE.mix(0.1)).label_font(("Calibri", 20)).draw(&mut backend).unwrap();
    ```
    ...
    */
    pub fn configure_series_labels<'b>(&'b mut self) -> SeriesLabelStyle<'b, 'e, CT> {
        SeriesLabelStyle::new(self)
    }

    /// Get a reference of underlying plotting area
    pub fn plotting_area(&self) -> &DrawingArea<CT> {
        &self.drawing_area
    }
    ...
    pub(crate) fn draw_series_impl<DB, B, E, R, S>(
        &mut self,
        backend: &mut DB,
        series: S,
        ...
        S: IntoIterator<Item = R>,
    {
        for element in series {
            self.drawing_area.draw(backend, element.borrow())?;
        }
        Ok(())
    }
    ...
    */
    ...
    where
        ...
        S: IntoIterator<Item = R>,
    {
        self.draw_series_impl(backend, series)?;
...
mod test {
    ...
    fn test_chart_context() {
        ...
        drawing_area.fill(&mut backend, &WHITE).expect("Fill");
        ...
        chart
            .draw_series(&mut backend, std::iter::once(Circle::new((5, 5), 5, &RED)))
            .expect("Drawing error");
        chart
            .draw_secondary_series(
                &mut backend,
                std::iter::once(Circle::new((0.3, 0.8), 5, &GREEN)),
            )
            .expect("Drawing error")
            .label("Test label")
    ...
    fn test_chart_context_3d() {
        ...
        let mut chart = ChartBuilder::on(&drawing_area)
            ...
            .set_label_area_size(LabelAreaPosition::Top, 20)
            .set_label_area_size(LabelAreaPosition::Right, 20)
            .build_cartesian_3d(&mut backend, 0..10, 0..10, 0..10)
            .expect("Create chart");
        ...
        chart
            .draw_series(
                &mut backend,
                std::iter::once(Circle::new((5, 5, 5), 5, &RED)),
            )
            .expect("Drawing error");
    }
}
