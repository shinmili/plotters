...
mod test {
    ...
    use plotters::prelude::{
        ChartBuilder, Color, IntoFont, SeriesLabelPosition, TextStyle, ToDrawingArea, BLACK, BLUE,
        RED, WHITE,
    };
    ...
    fn draw_mesh_with_custom_ticks(tick_size: i32, test_name: &str) {
        ...
        {
            let mut backend = SVGBackend::with_string(&mut content, (500, 500));
    ...
    fn test_text_draw() {
        let mut content: String = Default::default();
        {
            let mut backend = SVGBackend::with_string(&mut content, (1500, 800));
            let root = backend.to_drawing_area();
            let root = root
            ...
            let mut chart = ChartBuilder::on(&root)
                ...
                .set_all_label_area_size(40u32)
                .build_cartesian_2d(&mut backend, 0..100i32, 0..50i32)
                .unwrap();
    ...
    fn test_text_clipping() {
        let mut content: String = Default::default();
        {
            ...
            let mut backend = SVGBackend::with_string(&mut content, (width as u32, height as u32));
            let root = backend.to_drawing_area();
    ...
    fn test_series_labels() {
        let mut content = String::default();
        {
            ...
            let mut backend = SVGBackend::with_string(&mut content, (width, height));
            let root = backend.to_drawing_area();

            let mut chart = ChartBuilder::on(&root)
                .caption("All series label positions", ("sans-serif", 20u32))
                .set_all_label_area_size(40u32)
                .build_cartesian_2d(&mut backend, 0..50i32, 0..50i32)
                .unwrap();
            ...
            chart
                .draw_series(
                    &mut backend,
                    std::iter::once(Circle::new((5, 15), 5u32, &RED)),
                )
            ...
            chart
                .draw_series(
                    &mut backend,
                    std::iter::once(Circle::new((5, 15), 10u32, &BLUE)),
                )
                .expect("Drawing error")
                .label("Series 2")
